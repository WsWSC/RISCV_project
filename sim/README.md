...
| `perips_test/` | Local peripheral and MMIO smoke tests, ignored by git. | `python sim\perips_test\test_all.py` |
...
Run simulation commands sequentially when a runner uses `sim/inst_data.txt` or
`sim/out.vvp`; some standalone peripheral tests use temporary files.
