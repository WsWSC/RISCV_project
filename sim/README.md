...
Normal RV32I/RV32M regression binaries. These are run by `test_all.py` and `test_one_inst.py`.

```text
csr_test_bin/
```

CSR, trap, and interrupt regression binaries. These are run only by `test_csr.py`.
